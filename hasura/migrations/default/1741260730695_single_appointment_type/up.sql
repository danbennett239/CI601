...
CREATE OR REPLACE FUNCTION public.get_nearby_appointments(
  ...
  appointment_type text DEFAULT NULL, -- Single appointment type
  price_min numeric DEFAULT NULL, -- Optional min price for the type
  price_max numeric DEFAULT NULL, -- Optional max price for the type
...
WHERE
  ...
  -- Appointment type filter (check if key exists in services)
  AND (appointment_type IS NULL OR a.services ? appointment_type)
  -- Price filter on the specific appointment type's value
