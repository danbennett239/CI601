...
CREATE OR REPLACE FUNCTION public.get_nearby_appointments(
  ...
  appointment_type _text DEFAULT NULL, -- Array of appointment types, using _text for Hasura
...
WHERE
  ...
  -- Appointment type filter (match keys in services)
  AND (appointment_type IS NULL OR EXISTS (
    SELECT 1
    FROM jsonb_object_keys(a.services) AS key
    WHERE key = ANY(appointment_type)
  ))
  -- Price filter on services JSONB values
  AND (price_min IS NULL OR (
    SELECT MIN(value::numeric)
    FROM jsonb_object_keys(a.services) AS key,
    LATERAL jsonb_extract_path_text(a.services, key) AS value
    WHERE value ~ '^[0-9]+\\.?[0-9]*$' -- Ensure value is numeric
  ) >= price_min)
